#!/usr/bin/env python3
"""
score_csr_full_report_with_mismatch.py
...
1) For each CSR .afa alignment under sequences/CDS_CSR/<Genus>/aligned/:
     ...
     • For each other record (samples), compute:
         – Valid positions (skip '-' or 'N' in either seq)
         – Mismatch positions
         – Percent identity = matches/valid * 100
         – Mismatch count
2) Per species: average all sample percent‐IDs and mismatch‐counts, collect raw per-sample
   percent‐IDs into a comma‐separated string.
...
Usage:
   python score_csr_full_report_with_mismatch.py
"""
...
def percent_identity_and_mismatches(ref_seq: str, samp_seq: str):
    ...
    mcount = len(mismatches)
    return round(pid, 2), mcount, mismatches
...
def process_alignment(afa_path, detail_rows, summary_rows):
    ...
    # Detail: include reference as 100% identity, 0 mismatches
    ...
    for rec in records[1:]:
        ...
        if species == DETAIL_SPECIES:
            detail_rows.append({
                ...
                "Valid_Positions":    None,  # omit or recompute if desired
    ...
    avg_mpercent= round(100 - avg_pid, 2)  # overall mismatch percent
...
def main():
    ...
    # Compute weighted grand averages across all individuals
    ...
    weighted_avg_mpercent = round(100 - weighted_avg_pid, 2)

    # Append “All_species” row
    df_sum.loc[len(df_sum)] = {
        ...
        "Avg_Mismatch_Percent": weighted_avg_mpercent,
        "Raw_Percent_IDs":      ""
    }
    ...
    # Write detail for the chosen species
    if detail_rows:
