...
def ensure_directories_on_server(server, port, user, key_file):
    ...
    commands = [
        ...
        f'test -d {base_directory}{specie}/CDS_nucleotide/aligned || mkdir -p {base_directory}{specie}/CDS_nucleotide/aligned'
    ]
...
def execute_muscle_on_cds(server, port, user, key_file):
    ...
    aligned_directory = f"{base_directory}CDS_nucleotide/aligned/"
    ...
    try:
        ...
        output, error = execute_command_via_ssh(client, f"find {base_directory}CDS_nucleotide -type f -name '*.fasta'")
        if error:
...
# Function to upload nucleotide CDS files from local to server using SFTP
def upload_files(ssh_client, local_directory, remote_directory):
    sftp = ssh_client.open_sftp()
    try:
        # Walk through the local directory to find and upload nucleotide CDS files
        for root, _, files in os.walk(local_directory):
            for file in files:
                # Only upload .fasta files that are part of the Phocoena CDS_nucleotide directory
                if file.endswith('.fasta') and "Phocoena" in root and "CDS_nucleotide" in root:
...
def download_files(ssh_client, base_directory, local_directory):
    ...
    try:
        ...
        local_species_dir = os.path.join(local_directory, species_dir, "CDS_nucleotide", "aligned")
        remote_aligned_dir = f"{base_directory}{species_dir}/CDS_nucleotide/aligned"
...
# Upload CDS nucleotide files from local to server
