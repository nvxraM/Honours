...
def add_gaps_to_phocoena(base_dir):
    ...
    for filename in os.listdir(aligned_protein_dir):
        if filename.endswith('.afa'):  # Process only alignment files with '.afa' extension
            ...
            gene_name = filename.split('.')[0]  # Correctly extract gene name without extension
...
# Run the function to process only Phocoena
add_gaps_to_phocoena(main_dir)
